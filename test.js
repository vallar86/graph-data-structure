...
describe("Graph", function ()
{
	...
	describe("Dijkstra's Shortest Path Algorithm", function ()
	{
		...
		it("Should compute shortest path on two edges.", function ()
		{
			...
			assert.deepEqual(result, withWeight(["a", "b", "c"], 2));
		});
		...
		it("Should compute shortest path on example from Cormen text (p. 659).", function ()
		{
			...
			const result1 = graph.shortestPath("s", "z");
			assert.deepEqual(result1, withWeight(["s", "y", "z"], 5 + 2));
			const result2 = graph.shortestPath("s", "x");
			assert.deepEqual(result2, withWeight(["s", "y", "t", "x"], 5 + 3 + 1));
		});
	...
	{
		it("depthFirstSearch dummy test", function ()
		{
			var graph = Graph();
			graph.addEdge("a", "b");
			graph.addEdge("b", "c");
			...
			assert.equal(graph.depthFirstSearch(["a"], false, false)[0], "d");
			assert.equal(graph.depthFirstSearch(["d"], true, false)[0], "d");
